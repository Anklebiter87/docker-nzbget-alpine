...
#add nzbget user
RUN addgroup -g 1000 nzbget && adduser -H -D -G nzbget -s /bin/false -u 1000 nzbget

# create the install dir and volume mount points
RUN mkdir /nzbget && mkdir /config && mkdir /downloads && mkdir /completed && mkdir /setup

#will use this to check if a config is there or not
COPY config-check.sh /setup/

# download the latest nzbget version
RUN wget -O - http://nzbget.net/info/nzbget-version-linux.json | sed -n "s/^.*stable-download.*: \"\(.*\)\".*/\1/p" | wget --no-check-certificate -i - -O /setup/nzbget-latest-bin-linux.run
